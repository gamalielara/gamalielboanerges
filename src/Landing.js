        ...
        <section className="landing-page w-full h-screen">
            <div className="hello-container w-full h-full flex justify-start items-center bg-black relative">
                <div className="hello-content md:pl-16 md:p-0 p-4 z-10">
                    <motion.h3 className="lg:text-5xl md:text-3xl text-2xl text-gum-dark-red text-left font-semibold" initial={{opacity:0}} animate={{opacity:1}} transition={{duration:.75, delay:1.5}}>Hello, I am</motion.h3>
                    <motion.h1 className="lg:text-7xl md:text-5xl text-4xl text-gum-dark-red text-left font-bold" initial={{opacity:0, scale:0}} animate={{opacity:1, scale:1}} transition={{delay:.75, duration:.75}}>Ara Gamaliel <br/> Boanerges</motion.h1>
                    <div className="buttons flex gap-4 md:mt-8 sm:mt-4 mt-2">
                        <motion.button className="transition-all bg-transparent border-white border-2 hover:border-0 hover:border-transparent hover:bg-gum-dark-red text-white p-2" initial={{scale:0}} animate={{scale:1}} transition={{delay:1.25}}><Link to="/projects">MY WORKS</Link></motion.button>
                        <motion.button className="transition-all bg-transparent border-white border-2 hover:border-0 hover:border-transparent hover:bg-gum-dark-red text-white p-2" initial={{scale:0}} animate={{scale:1}} transition={{delay:1.5}}><Link to="/about-me">ABOUT ME</Link></motion.button>
                    </div>
