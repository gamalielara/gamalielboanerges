            ...
            <nav className="small-nav-menu fixed top-0 right-0 w-full sm:hidden transition-all ease-in-out header">
                <div className={clicked ? "full-menu w-screen h-screen bg-black p-4 relative" : "hidden"}>
                    ...
                    <div className="menus w-full h-full flex flex-col justify-center items-center gap-8">
                        <Link to="/"><button className="hover:bg-gum-red rounded py-1 px-2 transition-all" onClick={() => setClicked(false)}><motion.h1 className="text-lg text-white" initial={{opacity: 0, y:"100%"}} animate={clicked ? {opacity:1, y:0} : {opacity:0, y:"100%"}} transition={{duration: .2}}>Home</motion.h1></button></Link>
                        <Link to='/projects'><button className="hover:bg-gum-red rounded py-1 px-2 transition-all" onClick={() => setClicked(false)}><motion.h1 className="text-lg text-white" initial={{opacity: 0, y:"100%"}} animate={clicked ? {opacity:1, y:0} : {opacity:0, y:"100%"}} transition={{duration: .2, delay: .25}}>My Skills & Works</motion.h1></button></Link>
                        <Link to='/gumrindelwald'><button className="hover:bg-gum-red rounded py-1 px-2 transition-all" onClick={() => setClicked(false)}><motion.h1 className="text-lg text-white" initial={{opacity: 0, y:"100%"}} animate={clicked ? {opacity:1, y:0} : {opacity:0, y:"100%"}} transition={{duration: .2, delay: .5}}>My Blog</motion.h1></button></Link>
                        <Link to="/about-me"><button className="hover:bg-gum-red rounded py-1 px-2 transition-all" onClick={() => setClicked(false)}><motion.h1 className="text-lg text-white" initial={{opacity: 0, y:"100%"}} animate={clicked ? {opacity:1, y:0} : {opacity:0, y:"100%"}} transition={{duration: .2, delay: .75}}>About Me</motion.h1></button></Link>
                    </div>
                </div>
